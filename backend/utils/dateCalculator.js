 ...
 * @returns {{ dateRange: string, monday: Date, friday: Date }}
 */
export function getWeekRangeByYearWeek(year, week) {
  const firstMonday = getFirstMondayOfYear(year);
  ...
  const dateRange = `${monday.getMonth() + 1}.${monday.getDate()}-${friday.getMonth() + 1}.${friday.getDate()}`;
